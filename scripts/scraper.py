...
def scarica_partite():
    ...
    try:
        with sync_playwright() as p:
            ...
            # Cerca tutte le righe del palinsesto di oggi
            ...
            for row in rows:
                try:
                    ...
                    if lega_trovata:
                        # Estrai l'orario per l'ordinamento (es. 15:00, 18:45, 20:45)
                        time_match = re.search(r'\b(\d{1,2}:\d{2})\b', text)
                        orario_str = time_match.group(1) if time_match else "99:99"

                        # Pulizia del testo
                        righe = [r.strip() for r in text.split("\n") if r.strip()]
                        testo_completo = " - ".join(righe)
                        
                        # Scarta se contiene indicazioni di partite terminate o vecchie
                        if "disponibile on-demand" in testo_completo.lower():
                            continue
    ...
    # Ordina le partite in ordine CRONOLOGICO crescente per orario (es. 12:30 -> 15:00 -> 18:00 -> 20:45)
    ...
    print(f"Salvate {len(partite_trovate)} partite di oggi ordinate per orario in: {output_path}")
